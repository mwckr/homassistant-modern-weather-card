...
- Native Home Assistant UI Editor support
- Standard tap actions
- Dynamic upcoming forecast alerts

## Installation

### HACS (Coming Soon)
Installation via the Home Assistant Community Store (HACS) is planned and will be available soon.

### Manual Installation

1. Download the `modern-weather-card.js` and `modern-weather-card-editor.js` files from this repository.
2. Create a folder named `modern-weather-card` inside your Home Assistant `config/www` directory. 
...
4. Go to your Home Assistant dashboard, navigate to **Settings -> Dashboards -> Resources**.
5. Click **Add Resource** and enter the following URL:
   `/local/modern-weather-card/modern-weather-card.js`
   *(Optional: If you are upgrading from an older version, append the version number, for example `?v=0.1.0` at the end to try and bust the cache)*
6. Ensure the resource type is set to **JavaScript Module**.
7. Reload your browser or dashboard.

## Configuration

The card can be configured via the visual UI editor or using YAML.
...
| `sun_entity` | string | `sun.sun` | Entity used to determine time-of-day gradients. |
| `time_format` | string | `default` | Override time display: `default`, `12`, or `24`. |
...
| `tap_action` | object | `{ action: 'more-info' }` | Standard Home Assistant action triggered on physical tap. |
